...
module.exports = function loader(content) {
  ...
  var parsedResourceQuery = this.resourceQuery
  ...
  if (config.format) {
    if (!MIMES.hasOwnProperty(config.format)) {
      return loaderCallback(
        new Error('Format "' + config.format + '" not supported')
      );
    }
  ...
  var name = (config.name || "[hash]-[width].[ext]").replace(/\[ext\]/gi, ext);
  ...
  var sizes = parsedResourceQuery.size ||
  ...
  var createPlaceholder = function createPlaceholder(_ref3) {
    ...
    return JSON.stringify(
      "data:" + (mime ? mime + ";" : "") + "base64," + placeholder
    );
  };
